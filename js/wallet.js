...
wallet.config(function($stateProvider) {
    $stateProvider
        .state('main', {
            name: 'main',
            templateUrl: "partials/wallet/main.html",
            controller: function ($scope, $cookieStore, $http, Base64, $window) {
                ...
                angular.element(document).ready(function () {
                    ...
                    $scope.$watch('selectedAccount', function(newValue, oldValue) {
                        if (newValue != oldValue) {
                            $scope.run(newValue.account, qrcode);
                        }
                    });
                    $scope.$watch('qrCodeString', function(newValue, oldValue) {
                        qrcodeAllPurpose.makeCode(newValue);
                    });
                    $scope.run($scope.selectedAccount.account, qrcode);
                });
            }
        })
});

wallet.controller('walletController', ['$state', '$cookieStore', '$scope', 'appConfig', function ($state, $cookieStore, $scope, appConfig) {
    ...
    console.log('wallets: ' + $scope.wallets.length + ' w ' + $scope.wallet.key);
